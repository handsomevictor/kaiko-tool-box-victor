...
def get_data_single_exch(exch, pair, start_date, end_date, data_type, interval='1h'):
...
def get_data_single_exch_concurrent(exches, pair, start_date, end_date, interval, data_type, max_workers=20):
    temp = pd.DataFrame()
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        res = list(pool.map(get_data_single_exch, exches, repeat(pair), repeat(start_date), repeat(end_date),
...
def get_data_combined_exch_concurrent(exches, pairs, start_dates, end_dates, intervals, data_type, max_workers_process,
                                      max_workers_thread):
    """
